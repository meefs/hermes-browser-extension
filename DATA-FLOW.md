...
Hermes Browser Extension connects browser context to the Hermes Agent runtime you configure. This document describes the shipped v0.1.8 data flow.
...
v0.1.8 includes a collapsible “What Hermes saw” receipt after each sent turn. It summarizes:
...
When Hermes reports a tool call during a streaming turn, v0.1.8 renders it as an in-message Tool Activity Strip with a sanitized short preview. Tool names and previews are generated locally from runtime events; sensitive token shapes are redacted before display. Tool activity is UI state only and is not extra browser context sent to Hermes.
...
Before tab titles/URLs are included in the prompt, v0.1.8 redacts restricted categories such as browser internals, banking, crypto wallets, password managers, checkout/payment, health, and government tax/account pages.
...
The extension reads `/v1/capabilities` when available. If an older Hermes runtime does not expose that endpoint, v0.1.8 enters legacy compatibility mode:
...
v0.1.8 also separates gateway reachability from upstream Hermes runtime/tool tracebacks. If the API server is reachable but an upstream Hermes tool/runtime raises a Python traceback, the side panel can show a connected-with-warning diagnostic instead of treating the whole Browser connection as broken.
